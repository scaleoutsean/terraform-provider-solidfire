...
variable "solidfire_tenant_name" {
  type        = string
  default     = "test-account"
  description = "The Element tenant name."
}

variable "solidfire_tenant_target_secret" {
  type        = string
  sensitive   = true
  default     = ""
  description = "The Element tenant's CHAP target secret."
  validation {
    condition = (length(var.solidfire_tenant_target_secret) >= 12 && length(var.solidfire_tenant_target_secret) <= 16) || length(var.solidfire_tenant_target_secret) == 0
    # condition     = length([var.solidfire_tenant_target_secret]) >= 1
    error_message = "Tenant target secret must be between 12 to 16 ASCII characters in length."
  }
}

variable "solidfire_tenant_initiator_secret" {
  type        = string
  sensitive   = true
  default     = ""
  description = "The Element tenant's CHAP initiator secret."
  validation {
    condition = (length(var.solidfire_tenant_initiator_secret) >= 12 && length(var.solidfire_tenant_initiator_secret) <= 16) || length(var.solidfire_tenant_initiator_secret) == 0
    # condition     = length([var.solidfire_tenant_initiator_secret]) >= 12
